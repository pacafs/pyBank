...
def transfer_menu(bank, terminal_interface):
    ...
    if accounts:
        ...
        recipient = bank.db.fetchall("SELECT * FROM customers WHERE email=%s", (recipient_email,))
        if recipient:
            recipient_data = recipient[0]
            recipient_id = recipient_data[0]
            recipient_accounts = bank.db.fetchall("SELECT * FROM accounts WHERE customer_id=%s", (recipient_id,))
